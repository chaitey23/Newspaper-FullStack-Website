...
import { FaSearch, FaFilter, FaEye, FaTrash, FaSpinner, FaExclamationTriangle, FaNewspaper, FaCheck, FaTimes, FaClock, FaCommentAlt, FaCrown, FaRegStar, FaStar, FaEllipsisV } from 'react-icons/fa';
...
const DashBoardAllArticles = () => {
    ...
    const [actionDropdown, setActionDropdown] = useState(null); // For dropdown menu
    const limit = 10;
    ...
    const toggleActionDropdown = (articleId) => {
        setActionDropdown(actionDropdown === articleId ? null : articleId);
    };
    ...
    const handleRejectClick = (article) => {
        ...
        setIsModalOpen(true);
        setActionDropdown(null); // Close dropdown
    };
    ...
    const handleTogglePremium = async (articleId, currentPremiumStatus, articleTitle) => {
        try {
            ...
            const result = await Swal.fire({
                ...
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: currentPremiumStatus ? '#6b7280' : '#c99e66',
                cancelButtonColor: '#6b7280',
            ...
            if (result.isConfirmed) {
                const res = await axiosSecure.patch(`/admin/articles/${articleId}/premium`, {
                    isPremium: !currentPremiumStatus
                });

                if (res.data.message.includes('updated')) {
                    Swal.fire('Success!', `Article has been ${action}!`, 'success');
                    refetch();
                }
            }
            setActionDropdown(null); // Close dropdown
                                    ...
                                    <tr key={article._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="max-w-xs">
                                                ...
                                                {article.isPremium && (
                                                    <span className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-2 py-1 rounded-full text-xs font-bold mt-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className="relative">
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            toggleActionDropdown(article._id);
                                                        }}
                                                    ...
                                                    {/* Dropdown Menu */}
                                                    {actionDropdown === article._id && (
                                                        <div className="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-slate-200 z-10">
                                                            <div className="py-1">
                                                                <button
                                                                    onClick={() => handleTogglePremium(article._id, article.isPremium, article.title)}
                                                                    className={`flex items-center gap-2 w-full px-4 py-2 text-sm ${article.isPremium
                                                                        ? 'text-amber-600 hover:bg-amber-50'
                                                                        : 'text-purple-600 hover:bg-purple-50'
                                                                        } transition-colors`}
                                                                >
                                                                    {article.isPremium ? <FaStar /> : <FaRegStar />}
